...
func main() {
	...
	fmt.Printf(`::set-output name=latest_tag::%s name=new_major=::%s name=new_minor=::%s name=new_patch=::%s name=new_v_major=::v%s name=new_v_minor=::v%s name=new_v_patch=::v%s`, latestTag, newMajor, newMinor, newPatch, newMajor, newMinor, newPatch)
